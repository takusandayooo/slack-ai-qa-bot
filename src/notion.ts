...
export const searchNotionById = (notionId: string) => {
    ...
    const notionData = data.find((result: any) => `${result.properties.ID.unique_id.prefix}-${result.properties.ID.unique_id.number}` === notionId);
    if (!notionData) {
        return false;
    } else {
...
export const getNotionData = (pageId: string) => {
    ...
    const pageData = blockData.results.map((result: any) => {
