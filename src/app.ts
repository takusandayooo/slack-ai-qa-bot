...
import { addRecordToNotion, getNotionData, searchNotionById } from "./notion";
import { chatGpt, embeddingFromChatGpt } from "./openai";
import { addPineconeData, searchPineconeData } from "./pinecone";
import { getChannelNameFromSlackChannelId, getConversationList, getReplyMessage, postReplyMessage } from "./slackSDK";
...
export function main() {
  ...
  const flagMessage = messages.map((message) => {
    if (message.reactions) {
      ...
      if (botDoneStamp) {
        //NOTE: BOTがNotionに記録したスタンプ
        return { messageObject: message, flag: "botDoneStamp" };
      } else if (kanryouStamp) {
        //NOTE: ユーザがかんりょうとスタンプを押した場合
...
const replyMessageByRAG = (slackClient: SlackClient, message: string, ts: string) => {
  ...
  const channel = getChannelNameFromSlackChannelId(slackClient, config.SLACK_CHANNEL_TO_POST);
  Utilities.sleep(10);
  const pineceQuery = searchPineconeData(embeddingFromChatGpt(message)) as {
    id: string;
    score: number;
    values: number[];
  }[];
...
const recordDatabaseByNotion = (SlackClient: SlackClient, channel: string, ts: string) => {
  ...
  addPineconeData(
    embeddingFromChatGpt(replies[0].text),
    `${result.properties.ID.unique_id.prefix}-${result.properties.ID.unique_id.number}`,
  );
